...
# ═══════════════════════════════════════════════════════════════
# v2.6.0 extended: record_chat_complete — full pipeline entry
# ═══════════════════════════════════════════════════════════════

def record_chat_complete(
    chatlog_store: "ChatlogStore",
    timeline_store: "TimelineStore",
    user_id: str,
    user_message: str,
    ...
    *,
    ...
    auto_distill_interval: int = 10,
) -> dict:
    """Extended record_chat with optional distill/episodic/fact pipeline.

    Same as record_chat() but also runs:
    - Distiller.save_distill() every N turns (auto_distill_interval)
    - EpisodicStore (LLM-driven, not auto — returns flag for caller)
    - FactStore.save() for key-value facts (manual, not auto)

    The extended modules are opt-in — pass None to skip.
    ...
    Args:
        ...
        assistant_message: LLM-generated response
        distill: Distiller instance (or None to skip)
        episodic: EpisodicStore instance (or None to skip)
        facts: FactStore instance (or None to skip)
        auto_distill_interval: run distill every N turns (0 = never auto)

    Returns:
        dict with pipeline results:
            {wrote: bool, hash: str, distill_ran: bool, needs_episodic: bool}
    """
    from datetime import datetime

    # 1. Core: write conversation + timeline (same as record_chat)
    ok, msg_hash = chatlog_store.append(user_id, user_message)
    chatlog_store.append(assistant_id, assistant_message)
    ...
    result = {"wrote": ok, "hash": msg_hash, "distill_ran": False,
              "needs_episodic": False}

    # 2. Extended: auto-distill every N turns
    if distill and auto_distill_interval > 0:
        today = datetime.now().strftime("%Y-%m-%d")
        today_count = chatlog_store.count_day(today)
        if today_count % auto_distill_interval == 0 and today_count > 0:
            distill.save_distill(today)
            result["distill_ran"] = True

    # 3. Extended: check if episodic is needed
    if episodic:
        today = datetime.now().strftime("%Y-%m-%d")
        if episodic.load(today) is None and distill:
            d = distill.load_distill(today)
            if d and d.get("turns", 0) >= 5:
                result["needs_episodic"] = True
